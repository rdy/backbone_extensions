  ...
  function mixin(namespace) {
    ...
    function parseAssociation(associationType, associationName, options) {
      ...
      function through(response) {
        if (options.through) {
          var parsed = parseKey(_(options).result('through'), response);
          return (parsed && (associationType === 'hasOne' && parsed[options.parseKey || _(associationName).singularize()] ||
              _(parsed).pluck(options.parseKey || _(associationName).singularize())));
        }
      }
