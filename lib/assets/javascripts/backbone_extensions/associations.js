  ...
  function mixin(namespace) {
    ...
    function parseAssociation(associationType, associationName, options) {
      ...
      function through(response) {
        if (options.through) {
          var t = parseKey(_(options).result('through'), response), singularAssociationName = t && _(associationName).singularize();
          return singularAssociationName && (associationType === 'hasOne' && t[options.parseKey || singularAssociationName] ||
              _(t).pluck(options.parseKey || singularAssociationName));
        }
      }
        ...
        var parseFunc = _(options.parse).isFunction() && options.parse ||
            function(response) {
              return through.call(this, response) ||
                  ...
                  parseKey(associationName, response);
            };
    ...
    return {
      included: function(source) {
        var associations = _({belongsTo: {parse: false}, hasMany: {parse: true}, hasOne: {parse: true}}).reduce(function(associations, defaultOptions, associationType) {
