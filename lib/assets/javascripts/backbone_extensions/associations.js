    ...
    _(fn).extend({
      mergeOptions: function() {
        return _(arguments).chain().toArray().reduce(function(result, options) { return _(result).extend(options); }, {})
            .omit('class', 'className', 'lazy', 'inverseOf', 'parse', 'parseName', 'through').value();
      },
      ...
      parseAssociation: function(associationType, associationName, options) {
        function addParser(associations, parsers) {
          ...
          function wrapParsers(parsers, parsedResponse, association) {
            var self = this;
            _(parsers)
                .chain()
                .map(function(parser) {
                  return _(parser.parseFn.call(self).call(self, parsedResponse)).tap(function(result) {
                    parser.associationFn.call(self, result.response, association);
                  }).key;
                })
                .each(function(key) {
          ...
          if (_(parsers).chain().where({type: 'lazy'}).isEmpty().value()) {
            if (options.lazy) {
              var parse = function(oldAssociation) {
                wrapParsers.call(this, parsers, _(this.attributes).clone(), oldAssociation);
              }, memoizedParse = _(parse).memoize(function() {
                return associationName + this.cid;
              });

              this.prototype[associationName] = _(this.prototype[associationName]).wrap(function(oldAssociation, options) {
                var self = this;
                return _(oldAssociation.call(this)).tap(function() {
                  if ((options || {}).parse) {
                    parse.call(self, oldAssociation);
                  } else {
                    memoizedParse.call(self, oldAssociation);
                  }
                });
              });
            }
          ...
          if (_(parsers).chain().where({type: 'default'}).isEmpty().value()) {
            if (!options.lazy) {
              this.prototype.parse = _(this.prototype.parse).wrap(function(oldParse, response) {
                var self = this;
                return _(oldParse.call(this, response)).tap(function(parsedResponse) {
                  wrapParsers.call(self, parsers, parsedResponse);
                });
              });
            }
          }
          ...
          parsers.push({
            type: options.lazy && 'lazy' || 'default',
            parseFn: function() {
            ...
            associationFn: function(assocResponse, association) {
              return assocResponse &&
                  associations[associationType].call(this, assocResponse, association && association.call(this) || this[associationName](),
                      fn.mergeOptions(globalOptions, options, this._options));
            }
          });
        }

        if (options.parse || options.lazy) {
          ...
          if (associations[associationType]) {
            addParser.call(this, associations, this._parsers);
          }
        }
      }
    });
