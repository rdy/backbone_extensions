    ...
    function buildAssociation(associationType, Klass, associationName, options) {
      ...
      function createAssociation() {
        ...
        return (options['class'] && (new options['class'](null, newOptions))) ||
            (options.className && namespace[options.className] && new namespace[options.className](null, newOptions)) ||
    ...
    function parseAssociation(Klass, associationName, options, callback) {
      if (options.parse) {
        var parseFunc = _(options.parse).isFunction() ? options.parse : function(response) {
              return (options.className && response[options.className]) ||
                  (options.className && response[_.str.underscored(options.className)]) ||
                  response[associationName] ||
                  response[_.str.underscored(assocationName)];
            },
