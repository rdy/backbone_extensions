  ...
  function mixin(namespace) {
    ...
    function buildAssociation(associationType, associationName, options) {
      function through(association) {
        function throughAssociation() {
          var t = _(options.through).isFunction() && options.through.call(this) || this[_.str.camelize(options.through)]();
          return t[associationName] && t[associationName]();
        }
        return options.through && (association || throughAssociation.call(this));
      }
    ...
    function parseAssociation(associationType, associationName, options) {
      ...
      if (options.parse) {
        ...
        var parseFunc = _(options.parse).isFunction() && options.parse ||
            function(response) {
              return (options.through && (associationType === 'hasOne' && parseKey(_(options).result('through'), response)[_(associationName).singularize()] ||
                  _(parseKey(_(options).result('through'), response)).pluck(_(associationName).singularize()))) ||
