  ...
  function mixin(namespace) {
    ...
    function parseAssociation(associationType, associationName, options) {
      ...
      function through(response) {
        var t = options.through && parseKey(_(options).result('through'), response), singularAssociationName = t && _(associationName).singularize();
    ...
    return {
      included: function(source) {
        var associations = _({
          belongsTo: {parse: false}, hasMany: {parse: true}, hasOne: {parse: true}
