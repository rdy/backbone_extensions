...
(function(_, Backbone) {
  ...
  function mixin(namespace) {
    ...
    function parseAssociation(associationType, associationName, options) {
      function parseName(key, response) {
        return response[_.str.camelize(key)] || response[_.str.underscored(key)];
      }

      function through(response) {
        var t = options.through && parseName(_(options).result('through'), response), singularAssociationName = t && _.singularize && _(associationName).singularize();
        return singularAssociationName && (associationType === 'hasOne' && t[options.parseName || singularAssociationName] ||
            _(t).pluck(options.parseName || singularAssociationName));
      }
          ...
          var parseFunc = _(options.parse).isFunction() && options.parse ||
              function(response) {
                return through.call(this, response) ||
                    (options.parseName && parseName(options.parseName, response)) ||
                    (options.className && parseName(options.className, response)) ||
                    parseName(associationName, response);
              };
