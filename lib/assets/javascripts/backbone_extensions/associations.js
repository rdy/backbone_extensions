  ...
  function mixin(namespace) {
    ...
    function buildAssociation(associationType, associationName, options) {
      function through(association) {
        if (options.through) {
          var throughAssociation = _(options.through).isFunction() && options.through.call(this) || this[_.str.camelize(options.through)]();
          return (association &&
              association.add(throughAssociation.invoke(_(associationName).singularize()), mergeAssociationOptions({parse: true}, options, this.options))) ||
              throughAssociation[associationName]();
        }
      }
